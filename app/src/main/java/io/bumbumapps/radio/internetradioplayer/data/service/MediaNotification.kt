...
import io.bumbumapps.radio.internetradioplayer.data.service.extensions.PlayerActions
import io.bumbumapps.radio.internetradioplayer.extensions.notificationManager
...
class MediaNotification(private val service: PlayerService,
    ...
    private fun setupActions(builder: NotificationCompat.Builder, playbackState: PlaybackStateCompat) {
        ...
        if (isSkipEnabled) builder.addAction(generateAction(R.drawable.ic_base_skip_previous, "Previous", previousIntent))
        if (playbackState.state == PlaybackStateCompat.STATE_STOPPED
                || playbackState.state == PlaybackStateCompat.STATE_PAUSED) {
            builder.addAction(generateAction(R.drawable.ic_basel_play, "Play", playPauseIntent))
        } else {
            builder.addAction(generateAction(R.drawable.ic_base_pause, "Pause", playPauseIntent))
        }
        if (isSkipEnabled) builder.addAction(generateAction(R.drawable.ic_basel_skip_next, "Next", nextIntent))
    ...
    private fun createBuilder(): NotificationCompat.Builder {
        return NotificationCompat.Builder(service, CHANNEL_ID)
                ...
                .setSmallIcon(R.mipmap.radio_app_foreground)
